...
BinaryTreeNode<int>* buildTreeHelper(int* in, int* post, int inS, int inE, int postS, int postE) {
	...
	int lPostS = postS;
	int lPostE = lInE - lInS + lPostS; // as lInE - lInS = lPreE -iInS
	int rInS = rootIndex + 1;
	int rInE = inE;
	int rPostS = lPostE + 1;
	int rPostE = postE-1;
	BinaryTreeNode<int>*root = new BinaryTreeNode<int>(rootData);
	root->left = buildTreeHelper(in, post, lInS, lInE, lPostS, lPostE); // jaise binary tree bnane ke liye liye Pre and In chahiye the , vaise hi left subtree ke liye lPre and lIn chahiye .
